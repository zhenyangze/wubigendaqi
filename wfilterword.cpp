...
#include "ui_wfilterword.h"

WfilterWord::WfilterWord(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::WfilterWord)
{
    ui->setupUi(this);

    //ini文件
    this->iniFile = QDir::currentPath() + "/data/filter_signal.ini";
...
WfilterWord::~WfilterWord()
{
    delete ui;
}

//添加文件列表
void WfilterWord::on_pushButton_addFile_clicked()
{
...
//选择文件
void WfilterWord::selectItemFIle(QListWidgetItem *item){
...
//点击发送按钮，组合数据，传递给全局类
void WfilterWord::on_pushButton_send_clicked()
{
    this->saveIni();
    //filePath
...
//选择不限时间的按钮时，禁止时间输入框
void WfilterWord::checkInputTime(bool isSelect){
    if(isSelect == true) {
