...
#include "ui_wfiltercentence.h"

WfilterCentence::WfilterCentence(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::WfilterCentence)
{
    ui->setupUi(this);
...
WfilterCentence::~WfilterCentence()
{
    delete ui;
}
...
//添加文件列表
void WfilterCentence::on_pushButton_addFile_clicked()
{
...
//选择文件
void WfilterCentence::selectItemFIle(QListWidgetItem *item){
...
//点击发送按钮，组合数据，传递给全局类
void WfilterCentence::on_pushButton_send_clicked()
{
    this->saveIni();
    //filePath
...
//选择不限时间的按钮时，禁止时间输入框
void WfilterCentence::checkInputTime(bool isSelect){
    if(isSelect == true) {
